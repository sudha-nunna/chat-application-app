...
import { FiMessageSquare, FiCopy, FiExternalLink, FiLock, FiAlertCircle, FiArrowRight, FiUser, FiSun, FiMoon } from "react-icons/fi";
...
const SharedChatPage = () => {
  ...
  return (
    <div className={`h-full w-full overflow-y-auto flex flex-col ${isDark ? "bg-[#0f1015] text-white" : "bg-[#f8f9fc] text-text-primary"}`}>
      {/* Top Navigation Bar */}
      ...
      <main className="flex-1 w-full max-w-[840px] mx-auto px-4 py-6 sm:py-8 space-y-6">
        {messages.length === 0 ? (
      ...
      {/* Footer Banner */}
      <footer className="py-6 text-center text-xs text-text-muted border-t border-border-primary/40 dark:border-white/5">
        <p>This is a shared conversation from Codegene AI. To continue this chat, click "Continue in My Chats".</p>
      </footer>
    </div>
  );
};
