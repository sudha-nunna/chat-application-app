  ...
  return (
    <div className="flex flex-col h-full min-h-0 flex-1">
      ...
      >
        ...
        {/* Scheduled Intelligence & Notifications Trigger Button */}
        <button
          type="button"
          onClick={() => window.dispatchEvent(new CustomEvent("open-notification-center"))}
          className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl transition-all cursor-pointer text-xs font-semibold text-indigo-600 dark:text-indigo-400 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/20 group ${
            isSidebarCollapsed ? "justify-center" : "justify-between"
          }`}
          title="Scheduled Intelligence & Topic Notifications"
        >
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="relative flex items-center justify-center shrink-0">
              <FiBell className="text-base shrink-0 text-indigo-500 dark:text-indigo-400 group-hover:scale-110 transition-transform" />
            </div>
            {!isSidebarCollapsed && <span className="truncate">Scheduled Intelligence</span>}
          </div>
          {!isSidebarCollapsed && (
