...
const MessageBubble = ({ role, content, onRetry, isCreditWarning, isDailyLimit, onRecharge }) => {
  ...
  const hasCreditNotice = isCreditWarning || (content && typeof content === "string" && (
    content.includes("Insufficient Credits") ||
    content.includes("Daily Limit Reached") ||
    content.includes("top up your wallet") ||
    content.includes("recharge your wallet")
  ));
  ...
  return (
    <div className={`flex items-start gap-1.5 lg:gap-3 ${isUser ? "flex-row-reverse ml-auto max-w-[85%]" : "mr-auto w-full"} my-2.5 min-w-0`}>
      ...
      <div
        className={`min-w-0 max-w-full rounded-2xl p-3 lg:p-5 shadow-sm leading-relaxed text-[14px] overflow-hidden break-words [overflow-wrap:anywhere] [word-break:break-word] ${
          isUser
            ...
            : hasCreditNotice
            ? "bg-amber-500/5 border border-amber-500/30 text-text-primary dark:text-text-muted rounded-tl-none ring-1 ring-amber-500/20"
      ...
      >
        ...
        {/* ChatGPT / Claude Style Interactive Credit Depleted Banner */}
        {hasCreditNotice && (
          <div className="mt-3 p-3 rounded-xl border border-amber-500/30 bg-amber-500/10 dark:bg-amber-500/15 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-xs font-semibold text-amber-400">
              <FiAlertTriangle className="text-amber-400 text-sm shrink-0" />
              <span>{isDailyLimit ? "Daily limit reached (50 msgs/day)." : "Wallet balance depleted (0 cr)."}</span>
            </div>
            {onRecharge && (
              <button
                type="button"
                onClick={onRecharge}
                className="px-3 py-1.5 bg-amber-400 hover:bg-amber-300 text-black font-bold rounded-lg text-xs transition-all flex items-center gap-1.5 shrink-0 shadow-md active:scale-95 cursor-pointer"
              >
                ⚡ Recharge Wallet
              </button>
            )}
